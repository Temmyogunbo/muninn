...
def _text_array_param_sql(param_name: str) -> str:
    """
    RDS Data API does not support arrayValue parameters. Bind a JSON array string
    and convert to PostgreSQL text[] in SQL.
    """
    return (
        f"COALESCE(ARRAY(SELECT json_array_elements_text(:{param_name}::json)), ARRAY[]::text[])"
    )
...
class DataAPIClient:
    ...
    def __init__(
        ...
        self.database = database or os.environ.get("AURORA_DATABASE", "muninn")
        ...
        self.region = os.environ.get("DEFAULT_AWS_REGION", "us-east-2")
    ...
    def insert(self, table: str, data: Dict, returning: str = None) -> str:
        ...
        for col in columns:
            ...
            elif isinstance(data[col], list) and (
                not data[col] or all(isinstance(x, str) for x in data[col])
            ):
                placeholders.append(_text_array_param_sql(col))
    ...
    def update(self, table: str, data: Dict, where: str, where_params: Dict = None) -> int:
        ...
        for col, val in data.items():
            ...
            elif isinstance(val, list) and (not val or all(isinstance(x, str) for x in val)):
                set_parts.append(f"{col} = {_text_array_param_sql(col)}")
    ...
    def _build_parameters(self, data: Dict) -> List[Dict]:
        ...
        for key, value in data.items():
            ...
            elif isinstance(value, list) and (not value or all(isinstance(x, str) for x in value)):
                # RDS Data API: arrayValue is not supported; send JSON text for ::json cast in SQL
                param["value"] = {"stringValue": json.dumps(value)}
    ...
    def _extract_value(self, field: Dict) -> Any:
        ...
        if field.get("isNull"):
            ...
            av = field["arrayValue"]
            if "stringValues" in av:
                return list(av["stringValues"])
            if "longValues" in av:
                return list(av["longValues"])
            if "doubleValues" in av:
                return list(av["doubleValues"])
            if "booleanValues" in av:
                return list(av["booleanValues"])
            return []
